...
#real data
x_real = """Game 1: 8 green, 4 red, 4 blue; 1 green, 6 red, 4 blue; 7 red, 4 green, 1 blue; 2 blue, 8 red, 8 green
...
Game 100: 5 green, 17 blue, 5 red; 15 blue; 13 green, 8 red, 3 blue; 16 blue, 15 green, 8 red; 16 green, 2 blue, 3 red""".splitlines()

#test data
x_test = """Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
...
for line in x_real:
    add = 0
    # split the line into the game id and the results
    game, results = line.split(': ')
    # split the results into the individual draws
...
for line in x_real:
    ...
    for draw in results:
        ...
        for col in colours:
            if 'red' in col:
                r.append(int(col.split()[0]))
            if 'green' in col:
                g.append(int(col.split()[0]))
            if 'blue' in col:
                b.append(int(col.split()[0]))
    r_max = max(r)
    g_max = max(g)
    b_max = max(b)
    power_line = r_max * g_max * b_max
    power += power_line
